...
export default async function AvailabilityOverridesPage() {
  ...
  return <main className="admin-page settings-page"><Link className="back-link" href="/admin/configuracoes"><ArrowLeft size={17} /> Voltar para configurações</Link><div className="editor-heading"><p className="eyebrow">Bloqueios e exceções</p><h1>Adapte a agenda sem perder o padrão.</h1><p>Bloqueie períodos pontuais ou defina um horário diferente para uma data específica.</p></div><section className="availability-layout"><article className="editor-card"><p className="eyebrow">Bloquear período</p><BlockForm defaultDate={todayInTimezone()} resourceId={resource.id} /></article><article className="editor-card"><p className="eyebrow">Exceção de uma data</p><ExceptionForm defaultDate={todayInTimezone()} resourceId={resource.id} /></article></section><section className="admin-card availability-list"><div className="card-heading"><div><p className="eyebrow">Próximas alterações</p><h2>O que foge do horário padrão</h2></div></div>{blocks.length || exceptions.length ? <div className="availability-items">{blocks.map((block) => <article key={block.id}><CalendarOff size={18} /><div><strong>{block.title}</strong><span>{formatDate(block.startsAt, { day: "2-digit", month: "long" })} · {formatTime(block.startsAt)} às {formatTime(block.endsAt)}</span></div><DeleteAvailabilityForm id={block.id} kind="block" /></article>)}{exceptions.map((exception) => <article key={exception.id}><Clock3 size={18} /><div><strong>{exception.isClosed ? "Studio fechado" : "Horário especial"}</strong><span>{formatDate(exception.date, { day: "2-digit", month: "long", timeZone: "UTC" })}{exception.isClosed ? " · dia inteiro" : ` · ${toTime(exception.startsAtMinute)} às ${toTime(exception.endsAtMinute)}`}{exception.note ? ` · ${exception.note}` : ""}</span></div><DeleteAvailabilityForm id={exception.id} kind="exception" /></article>)}</div> : <div className="empty-state"><p>Nenhuma alteração futura.</p><span>A rotina semanal está sendo usada sem exceções.</span></div>}</section></main>;
}
