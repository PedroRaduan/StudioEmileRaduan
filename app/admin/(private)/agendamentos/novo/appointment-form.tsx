...
export function AppointmentForm({ date, minDate, time, selectedClientId, selectedResourceId, clients, services, resources }: Props) {
  ...
  return <form action={action} className="editor-form">
    ...
    <div className="form-grid two-columns">
      ...
      <div className="field-group"><label htmlFor="time">Horário</label><input defaultValue={time} id="time" name="time" required step="300" type="time" /></div>
      <div className="field-group"><label htmlFor="resourceId">Agenda</label><select defaultValue={resources.some((resource) => resource.id === selectedResourceId) ? selectedResourceId : resources[0]?.id ?? ""} id="resourceId" name="resourceId" required>{resources.map((resource) => <option key={resource.id} value={resource.id}>{resource.name}</option>)}</select></div>
    </div>
