...
type Props = { date: string; minDate: string; clients: { id: string; fullName: string; preferredName: string | null }[]; services: { id: string; name: string; durationMinutes: number; priceCents: number | null; promotionalPriceCents: number | null }[]; resources: { id: string; name: string }[] };
const initialState: AppointmentFormState = {};

export function AppointmentForm({ date, minDate, clients, services, resources }: Props) {
  ...
  return <form action={action} className="editor-form">
    ...
    <div className="form-grid two-columns">
      <div className="field-group"><div className="field-label-row"><label htmlFor="clientId">Cliente</label><Link href="/admin/clientes/novo">Cadastrar nova</Link></div><select defaultValue="" id="clientId" name="clientId" required><option disabled value="">Selecione uma cliente</option>{clients.map((client) => <option key={client.id} value={client.id}>{client.preferredName ?? client.fullName}</option>)}</select></div>
      ...
      <div className="field-group"><label htmlFor="date">Data</label><input defaultValue={date} id="date" min={minDate} name="date" required type="date" /></div>
