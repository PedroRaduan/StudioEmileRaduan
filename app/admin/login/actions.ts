...
export async function loginAction(_: LoginState, formData: FormData): Promise<LoginState> {
  ...
  const membership = await prisma.organizationMembership.findFirst({ where: { userId: user.id, isActive: true, organization: { isActive: true } }, orderBy: { createdAt: "asc" } });
  if (!membership) return { error: "E-mail ou senha incorretos." };

  await prisma.$transaction([
    prisma.user.update({ where: { id: user.id }, data: { lastLoginAt: new Date() } }),
    prisma.auditLog.create({ data: { organizationId: membership.organizationId, userId: user.id, action: "ADMIN_LOGIN", entityType: "User", entityId: user.id, ipHash } }),
  ]);
