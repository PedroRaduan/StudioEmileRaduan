...
export default async function BookingSuccessPage({ searchParams }: { searchParams: Promise<{ appointment?: string }> }) {
  ...
  if (!id) notFound();
  const [item, settings] = await Promise.all([getPrisma().appointment.findFirst({ where: { id, clientId: current.clientId }, include: { service: true } }), getPrisma().studioSettings.findUnique({ where: { id: "studio" } })]);
  if (!item) notFound();
