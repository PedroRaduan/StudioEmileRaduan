...
export async function getAgendaForDay(date: string) {
  ...
  const settings = await prisma.studioSettings.findUnique({ where: { organizationId }, select: { calendarSlotInterval: true, timezone: true } });
...
export async function getAgendaTimezone() {
  await requirePermission("APPOINTMENTS_MANAGE");
  const { organizationId } = await requireTenantContext();
  const settings = await getPrisma().studioSettings.findUnique({ where: { organizationId }, select: { timezone: true } });
