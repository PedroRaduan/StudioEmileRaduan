...
const securityHeaders = [
  ...
  { key: "Content-Security-Policy", value: `default-src 'self'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'; object-src 'none'; img-src 'self' data: https:; script-src ${scriptSource}; style-src 'self' 'unsafe-inline'; connect-src ${connectSource}; manifest-src 'self'; worker-src 'self'` },
];
